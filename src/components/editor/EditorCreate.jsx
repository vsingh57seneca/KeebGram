...
const EditorCreate = ({ initialData }) => {
    ...
    const handleSaveDesign = async () => {
        ...
        // Capture image of design
        const KeyboardElement = document.getElementById('keyboard');
        html2canvas(KeyboardElement).then(canvas => {
            ...
            console.error('Error capturing keyboard:', error);
        });
        ...
        try {
            ...
            if (response && (response.status === 201 || response.status === 200)) {
                ...
                throw new Error("Failed to save design. Status code: " + response.status);
            }
        } catch (error) {
            console.error('Save design error:', error);
            alert(error.message || "An error occurred. Failed to save design.");
        }
    };
    ...
    <>
        <div className="m-2">
            ...
            <div className="flex space-x-4 mt-4">
                ...
                <button className="text-gray-900 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700" onClick={handleCancel}>Cancel</button>
            </div>
      </div>
    </>
  );
